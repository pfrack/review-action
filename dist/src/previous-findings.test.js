...
test('formatPreviousFindings: truncates at the thread cap with the marker', () => {
    const threads = Array.from({ length: 25 }, (_, i) => mkThread({ id: `PRRT_${i}`, body: `finding ${i}` }));
...
test('formatPreviousFindings: returns empty when a single thread exceeds the char cap entirely', () => {
    const out = formatPreviousFindings([mkThread({ body: 'x'.repeat(2000) })], 100, 1500);
    assert.equal(out, '');
});
...
test('formatPreviousFindings: returns empty when input is not an array', () => {
    assert.equal(formatPreviousFindings(null), '');
    assert.equal(formatPreviousFindings(undefined), '');
});
