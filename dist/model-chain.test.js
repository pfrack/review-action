...
describe('buildCombinedChain', () => {
    it('NIM-only: includes only NIM models when only NIM key is available', () => {
        const chain = buildCombinedChain(['deepseek-ai/deepseek-v4-pro', 'meta/llama-3.3-70b-instruct'], ['mistral-medium-3.5', 'codestral-2508'], true, false);
    ...
    it('Mistral-only: includes only Mistral models when only Mistral key is available', () => {
        const chain = buildCombinedChain(['deepseek-ai/deepseek-v4-pro'], ['mistralai/mistral-medium-3.5-128b', 'mistralai/mistral-small-4-119b-2603', 'nvidia/llama-3.3-nemotron-super-49b-v1'], false, true);
    ...
    it('combined: merges both lists sorted by SWE-bench score', () => {
        const chain = buildCombinedChain(['deepseek-ai/deepseek-v4-pro', 'meta/llama-3.3-70b-instruct'], ['mistralai/mistral-medium-3.5-128b', 'mistralai/mistral-small-4-119b-2603'], true, true);
    ...
    it('empty: returns empty array when neither key is available', () => {
        const chain = buildCombinedChain(['deepseek-ai/deepseek-v4-pro'], ['mistral-medium-3.5'], false, false);
    ...
    it('unknown models get default score 0.5', () => {
        const chain = buildCombinedChain(['unknown/model-a'], ['unknown-mistral-model'], true, true);
        assert.strictEqual(chain.length, 2);
    ...
    it('preserves order among models with same score', () => {
        ...
        const chain = buildCombinedChain(['mistralai/mistral-nemotron'], ['mistralai/mistral-large-3-675b-instruct-2512'], true, true);
        assert.strictEqual(chain.length, 2);
