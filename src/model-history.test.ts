...
import { loadHistory, saveHistory, detectNewModels, detectRemovedModels, updateHistory, type ModelHistory } from './model-history.js';
...
describe('loadHistory', () => {
  ...
  it('parses valid JSON file', () => {
    ...
    try {
      ...
      const data: ModelHistory = { nim: { models: ['model-a', 'model-b'] } };
...
describe('saveHistory', () => {
  it('writes pretty-printed JSON', () => {
    ...
    try {
      const path = join(tmpDir, 'history.json');
      const data: ModelHistory = { nim: { models: ['model-a'] } };
      saveHistory(data, path);
...
describe('detectNewModels', () => {
  it('returns models in catalog but not in history', () => {
    const history: ModelHistory = { nim: { models: ['model-a', 'model-b'] } };
  ...
  it('returns empty when all models are known', () => {
    const history: ModelHistory = { nim: { models: ['model-a'] } };
...
describe('detectRemovedModels', () => {
  it('returns models in history but not in catalog', () => {
    const history: ModelHistory = { nim: { models: ['model-a', 'model-b', 'model-c'] } };
...
describe('updateHistory', () => {
  ...
  it('replaces existing provider entry', () => {
    const history: ModelHistory = { nim: { models: ['old-model'] } };
