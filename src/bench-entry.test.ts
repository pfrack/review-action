...
import { BenchmarkResult } from './bench.js';
...
function makeActionYml(tmpDir: string, models: string[]): string {
...
describe('classifyFailedModels', () => {
  it('classifies probe-pass + catalog-listed as demoted with probe latency', async () => {
    const { demoted, transient, permanent } = await classifyFailedModels(
      ['nvidia/llama-3.1-nemotron-ultra-253b-v1'],
      async () => true,
      new Set(['nvidia/llama-3.1-nemotron-ultra-253b-v1']),
    );
  ...
  it('handles mixed failures and preserves classification per model', async () => {
    ...
    const { demoted, transient, permanent } = await classifyFailedModels(
      ['healthy-model', 'transient-model', 'gone-model'],
      async (m) => probeResults.get(m) ?? false,
      new Set(['healthy-model', 'transient-model']),
    );
...
describe('readmitCatalogModels', () => {
  ...
  it('respects the limit parameter', async () => {
    ...
    const proxy = {
      probeModel: async (m: string) => { probeCalls.push(m); return client.probeModel(m); },
      chat: client.chat.bind(client),
      chatStream: client.chatStream.bind(client),
    };
  ...
  it('re-admits models sorted by SWE-bench score descending', async () => {
    ...
    assert.deepStrictEqual(
      [...probeOrder].sort(),
      ['mistralai/mistral-large', 'nvidia/nemotron-3-super-120b-a12b'],
    );
  });
  ...
  it('respects the concurrency option by bounding in-flight calls', async () => {
    ...
    let inFlight = 0;
    let maxInFlight = 0;
    const track = async (fn: () => Promise<unknown>) => {
      inFlight += 1;
      maxInFlight = Math.max(maxInFlight, inFlight);
      try {
        return await fn();
