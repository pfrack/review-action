...
export async function runBenchmark(
  ...
  const messages: ChatMessage[] = [{ role: 'user', content: cfg.prompt }];
  const opts = { temperature: cfg.temperature, maxTokens: cfg.maxTokens };
  const result: BenchmarkResult = { model, iterations: [] };
...
export function countErrors(iters: IterationResult[]): number {
...
export function formatMarkdownTable(results: BenchmarkResult[]): string {
  ...
  for (const r of results) {
    ...
    for (const it of r.iterations) {
      ...
      if (it.ttft > 0) ttfts.push(it.ttft);
      latencies.push(it.latency);
      if (it.tokensPerSec > 0) tpsList.push(it.tokensPerSec);
    }
