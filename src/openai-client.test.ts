...
import { OpenAIClient, type ChatMessage, extractJsonFromText, stripThinkingContent, effectiveFormat, sanitizeErrorBody } from './openai-client.js';
...
describe('OpenAIClient provider label detection', () => {
  it('auto-detects OpenRouter label from base URL', () => {
    ...
    assert.strictEqual((client as any).providerLabel, 'OpenRouter');
  });

  it('auto-detects Kilo label from base URL', () => {
    const client = new OpenAIClient('https://api.kilo.ai/api/gateway', 'key');
    assert.strictEqual((client as any).providerLabel, 'Kilo');
  });
  ...
  it('auto-detects NousResearch label from base URL', () => {
    ...
    assert.strictEqual((client as any).providerLabel, 'NousResearch');
    assert.strictEqual((client as any).providerKey, 'nousresearch');
  });

  it('uses explicit label when provided', () => {
    const client = new OpenAIClient('https://openrouter.ai/api/v1', 'key', 'MyLabel');
    assert.strictEqual((client as any).providerLabel, 'MyLabel');
  });
});
...
describe('OpenAIClient', () => {
  ...
  it('extractJsonFromText: handles ```json fence, plain fence, bare object, and prose', () => {
    ...
    assert.strictEqual(
      extractJsonFromText('Here is the result: {"a":3, "b":[1,2]} and some trailing prose.'),
      '{"a":3, "b":[1,2]}',
    );
    // Nested objects — brace counter must not be fooled by inner braces
    assert.strictEqual(
      extractJsonFromText('noise {"a":{"b":2},"c":3} tail'),
      '{"a":{"b":2},"c":3}',
    );
    // Strings containing braces must not affect the counter
    assert.strictEqual(
      extractJsonFromText('{"a":"has } brace","b":2}'),
      '{"a":"has } brace","b":2}',
    );
    // Escaped quotes inside strings
    assert.strictEqual(
      extractJsonFromText('{"a":"escaped \\" quote","b":2}'),
      '{"a":"escaped \\" quote","b":2}',
    );
    // No JSON found
  ...
  it('extractJsonFromText: strips thinking blocks before JSON extraction', () => {
    ...
    assert.strictEqual(
      extractJsonFromText('<thinking>why { not } this</thinking>{"a":1}'),
      '{"a":1}',
    );
    // thinking with a bare JSON object after it
    assert.strictEqual(
      extractJsonFromText('[thinking]some reasoning[/thinking]\n{"summary":"ok","findings":[]}'),
      '{"summary":"ok","findings":[]}',
    );
    // === thinking === ... === answer === ... JSON
    assert.strictEqual(
      extractJsonFromText('=== thinking ===\nreasoning\n=== answer ===\n{"a":1}'),
      '{"a":1}',
    );
  });
  ...
  it('stripThinkingContent removes all supported dialects', () => {
    assert.strictEqual(
      stripThinkingContent('<thinking>reason</thinking>'),
      '',
    );
    assert.strictEqual(
      stripThinkingContent('[thinking]reason[/thinking]'),
    ...
    assert.strictEqual(
      stripThinkingContent('=== thinking ===\nreason\n=== answer ===\n{"a":1}'),
      '{"a":1}',
    );
    // Multiple blocks
    assert.strictEqual(
      stripThinkingContent('<thinking>a</thinking>content<thinking>b</thinking>'),
      'content',
    );
  ...
  it('probeModel returns permanent=true on 403', async () => {
    const mock = await startMockServer((_req, res) => {
      res.writeHead(403);
      res.end('forbidden');
    });
  ...
  it('probeModel returns permanent=true on 413', async () => {
    const mock = await startMockServer((_req, res) => {
      res.writeHead(413);
      res.end('too large');
    });
...
describe('OpenAIClient listModels', () => {
  ...
  it('throws when /models is not ok', async () => {
    ...
    try {
      ...
      await assert.rejects(
        () => client.listModels(),
        (err: Error) => err.message.includes('/models returned 500'),
      );
    } finally {
      mock.close();
    }
