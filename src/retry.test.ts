...
describe('withRetry', () => {
  it('retries on RetryableError with status 500', async () => {
    ...
    const result = await withRetry(() => {
      attempts++;
      if (attempts === 1) throw new RetryableError('internal error', 500);
  ...
  it('does not retry on RetryableError with status 400', async () => {
    ...
    await assert.rejects(
      ...
      (err: unknown) => err instanceof RetryableError && err.status === 400,
    );
  ...
  it('retries up to maxRetries then gives up', async () => {
    ...
    await assert.rejects(
      ...
      (err: unknown) => err instanceof RetryableError && err.status === 503,
    );
