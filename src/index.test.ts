...
import { computeMaxTokens, runModelChainForBatch, buildClients, buildRawOutputBody, type BatchResult, withAggregateTimeout, executeReview, prioritizeChain, run, dispatchOutput } from './index.js';
...
describe('dispatchOutput — comment_mode branch', () => {
  ...
  function makeDispatchFetch(posts: Post[], behavior: {
    listThreadsResponse?: unknown;
    listThreadsThrow?: boolean;
    ...
    const reviews = new Set<number>(behavior.existingReviewId != null ? [behavior.existingReviewId] : []);
    globalThis.fetch = (async (input: any, init?: any) => {
      ...
      if (url.includes('/graphql')) {
        ...
        if (bodyStr.includes('reviewThreads')) {
          return new Response(
            JSON.stringify(behavior.listThreadsResponse ?? { data: { repository: { pullRequest: { reviewThreads: { nodes: [] } } } } }),
  ...
  function makeReview(findingsCount: number, lineAnchored = true) {
  ...
  function makeContext(overrides: Partial<ReturnType<typeof makeConfig> & { commentMode: 'summary' | 'inline' }>, review: ReturnType<typeof makeReview>) {
  ...
  function captureWarnings() {
    ...
    const spy = (chunk: string | Uint8Array, ...args: unknown[]): boolean => {
      const s = typeof chunk === 'string' ? chunk : Buffer.from(chunk).toString('utf8');
      if (s.includes('::warning')) warnings.push(s);
      return (orig as (c: string | Uint8Array, ...a: unknown[]) => boolean)(chunk, ...args);
    };
    (process.stdout.write as unknown) = spy;
    return {
      warnings,
      restore: () => { (process.stdout.write as unknown) = orig; },
    };
  }
  ...
  it('inline mode posts a review (createReview), not a summary comment', async () => {
    ...
    try {
      await dispatchOutput(makeContext({ commentMode: 'inline' }, makeReview(2)) as any);
  ...
  it('inline mode falls back to summary comment when too many findings', async () => {
    ...
    try {
      await dispatchOutput(makeContext({ commentMode: 'inline' }, makeReview(60)) as any);
      const reviewPosts = posts.filter((p) => p.url.endsWith('/pulls/42/reviews'));
      const commentPosts = posts.filter((p) => p.url.endsWith('/issues/42/comments'));
  ...
  it('summary mode (default) posts exactly the summary comment path', async () => {
    ...
    try {
      await dispatchOutput(makeContext({ commentMode: 'summary' }, makeReview(2)) as any);
      const reviewPosts = posts.filter((p) => p.url.endsWith('/pulls/42/reviews'));
      const commentPosts = posts.filter((p) => p.url.endsWith('/issues/42/comments'));
    ...
    const clients: Record<Provider, OpenAIClient | null> = {
      nim: {
        probeModel: async (model: string) => {
          if (deadModels.has(model)) return { ok: false, permanent: true, status: 410 };
          return { ok: true, permanent: false };
        },
        chat: async (_model: string, _msgs: ChatMessage[], opts?: ChatOptions): Promise<ChatResult> => {
          if (opts?.signal?.aborted) {
