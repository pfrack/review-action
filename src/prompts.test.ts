...
describe('buildSystemMessage — replace mode security preservation', () => {
  ...
  it('replace-mode security section contains only focus areas, not the entire base prompt', () => {
    ...
    assert.ok(!securityBlock.includes('## Severity Classification'),
      'security block must not duplicate the severity guidance header');
    assert.ok(!securityBlock.includes('Anti-patterns (do NOT flag these)'),
      'security block must not include non-security anti-patterns content');
    assert.ok(!securityBlock.includes('expert Go engineer'),
      'security block must not duplicate the role description');
