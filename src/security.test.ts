...
describe('validateProviderUrl — SSRF blocklist', () => {
  it('blocks 169.254.169.254 (AWS/Azure metadata)', () => {
    assert.throws(
      () => validateProviderUrl('https://169.254.169.254/latest/meta-data/', 'custom_api_url'),
      /blocked.*link-local/,
    );
  });
  ...
  it('blocks metadata.google.internal', () => {
    assert.throws(
      () => validateProviderUrl('https://metadata.google.internal/computeMetadata/v1/', 'custom_api_url'),
  ...
  it('allows api.example.com', () => {
    assert.doesNotThrow(
      () => validateProviderUrl('https://api.example.com/v1', 'custom_api_url'),
    );
  });
  ...
  it('blocks other 169.254.x.x link-local addresses', () => {
    assert.throws(
      () => validateProviderUrl('https://169.254.0.1/path', 'test'),
      /blocked.*link-local/,
    );
  });
...
describe('loadConfig — max_files validation', () => {
  ...
  function setup(maxFiles: string) {
