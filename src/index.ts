...
export type BatchResult = {
  ...
  dropped: number;
  /** End-to-end latency of the underlying model call, in milliseconds. */
  latencyMs: number;
};
...
function sweScore(tagged: TaggedModel): number {
 ...
 */
function effectiveScore(tagged: TaggedModel, latencyMs: number): number {
  const swe = sweScore(tagged);
  if (latencyMs <= 60_000) return swe;
